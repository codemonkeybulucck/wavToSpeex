...
- (void)encapsulatingOver:(NSString *)wavPath{
    
//    NSLog(@"wavpath:%@",wavPath);
    
    if (self.delegate) {
        [self.delegate recordingFinishedWithFileName:filename wavPath:wavPath time:[self recordedTimeInterval]];
    }
}
...
- (void)updateLevelMeter:(id)sender {
    if (self.delegate) {
        ...
        AudioQueueGetProperty(mAQRecorder->Queue(), kAudioQueueProperty_CurrentLevelMeterDB, levelMeterStates, &dataSize);
